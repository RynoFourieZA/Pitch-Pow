...
const Questions_New = () => {
    ...
    async function getNewQuestions() {
		try {
            ...
            const newData = await parseRes.filter(item => item.pitch_type_name === "New");
			// "Business Description"
			const getBusinessDescription = newData.filter(questions => questions.name === "Business Description").map(question => {
			...
			// "Innovation"
			const getInnovation = newData.filter(questions => questions.name === "Innovation").map(question => {
				delete question.name;
				return question;
			});
			...
			// Market Analysis
			const getMA = newData.filter(questions => questions.name === "Market Analysis").map(question => {
				delete question.name;
				return question;
			});
			...
			const getPSA = newData.filter(questions => questions.name === "Product or Service Analysis").map(question => {
				delete question.name;
				return question;
			});
			...
			// Competition
			const getCompetition = newData.filter(questions => questions.name === "Competition").map(question => {
				delete question.name;
				return question;
			});
			...
			// Marketing Strategy
			const getMS = newData.filter(questions => questions.name === "Marketing Strategy").map(question => {
				delete question.name;
				return question;
			});
			...
			// Operations
			const getOperations = newData.filter(questions => questions.name === "Operations").map(question => {
				delete question.name;
				return question;
			});
			...
			// Finances
			const getFinances = newData.filter(questions => questions.name === "Finances").map(question => {
				delete question.name;
				return question;
			});
			...
			const getMTc = newData.filter(questions => questions.name === "Management/Technical complexity").map(question => {
				delete question.name;
				return question;
			});
			...
			const allTheObjects = [...obj1, ...obj2, ...obj3, ...obj4, ...obj5, ...obj6, ...obj7, ...obj8, ...obj9];
